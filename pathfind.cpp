...
// to destination
void pathFinder::tracePath(cell cellDetails[][COL], Pair dest, int &Y, int &X)
{
    Y = dest.first;
    X = dest.second;
    ...
    while (!(cellDetails[Y][X].parent_i == Y && cellDetails[Y][X].parent_j == X))
    {
        Path.push(std::make_pair(Y, X));
        int temp_row = cellDetails[Y][X].parent_i;
        int temp_col = cellDetails[Y][X].parent_j;
        Y = temp_row;
        X = temp_col;
    }
...
void pathFinder::aStarSearch(int grid[][COL], Pair src, Pair dest, int &nY, int &nX)
{
